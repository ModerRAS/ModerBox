---
description: "Sync latest master, work on a fresh branch, create or update a PR, inspect CI and PR feedback, then report the final state."
---
...
7. If work is blocked by missing permissions, an external outage, or ambiguous requirements, explain the blocker clearly in the final response.

## Additional guidance

- Prefer a clean branch from latest `master` for new work.
- Keep edits focused on the requested scope.
- Use the repository's real commands, paths, and workflows instead of generic placeholders.
